...
@Injectable()
export class AdditionProductsManager extends MyDatatableManager {
    ...
    public loadEntities() {
        ...
        this.getBackend().get(url)
            .then((resp: Response) => {
                ...
                var _self = this;
                setTimeout(function(){
                    _self.initDraggable($('addition-catalog-products table.dataTable tbody'));
                }, 0);
    ...
    protected getEntityFromData(data:any): MyDatatableEntity {
        var product = new AdditionProduct(data['id']);
