...
@Component({
    selector: 'addition-categories-tree',
    templateUrl: './addition-categories-tree.html',
    styleUrls: ['./addition-categories-tree.css'],
    encapsulation: ViewEncapsulation.None,
})
export class AdditionCategoriesTreeComponent implements OnInit {

    private tree: any = null;

    private rootCategory: AdditionCategory;

    private selectedCategories: AdditionCategory[] = [];
    ...
    @Output()
    public onCreate: EventEmitter<AdditionCategory> = new EventEmitter<AdditionCategory>();

    @Output()
    public onUpdate: EventEmitter<AdditionCategory> = new EventEmitter<AdditionCategory>();

    @Output()
    public onDelete: EventEmitter<AdditionCategory> = new EventEmitter<AdditionCategory>();
    ...
    public constructor(private eRef: ElementRef) {
        this.rootCategory = new AdditionCategory();
        this.rootCategory.id = -2;
    ...
    public ngOnInit() {
        ...
        $cont.on('click', '.addition-leaf-controls__create', (event: Event) => {
        ...
        $cont.on('click', '.addition-leaf-controls__update', (event: Event) => {
            var category = this.getCategoryFromUserEvent(event);
            this.onUpdate.emit(category);
            return false;
        });

        $cont.on('click', '.addition-leaf-controls__delete', (event: Event) => {
    ...
    public load(categories: AdditionCategory[], selectedCategories: AdditionCategory[]) {
        if(this.tree) {
            this.tree.destroy();
        }
        this.selectedCategories = selectedCategories;
        ...
        if(this.enableSorting && !this.sortBound) {
            ...
            $(document).on('dnd_stop.vakata', (e: any, data: any) => {
                if($(data.data.obj.context).find('[aria-level="1"]').attr('id') != -2) {
                    return false;
                }
                var tree = this.tree;
    ...
    private getCategoryFromUserEvent(event: Event): AdditionCategory {
        var id = $(event.target).closest('.leaf').data('id');
        return this.getCategoryById(id);
    }

    private getCategoryById(id: number): AdditionCategory {
        if(id == this.rootCategory.id) {
            return null;
        }
        var findFromCategory = (category: AdditionCategory, id: number): AdditionCategory => {
            if(category.id == id) {
                return category;
            }
...
class TreeNode {
    ...
    public constructor(private category: AdditionCategory, isRoot: boolean, enableControls: boolean, selectedCategories: AdditionCategory[]) {
        this.id = category.id;
        var controls = `
            <button title="Добавить подкатегорию" type="button" class="btn bg-success btn-icon addition-leaf-controls__create">
                <i class="icon-plus2"></i>
            </button>
        `;
        if(!isRoot) {
            controls += `
                <button title="Редактировать" type="button" class="btn bg-info-600 btn-icon addition-leaf-controls__update">
                    <i class="icon-pen"></i>
                </button>
                <button title="Удалить" type="button" class="btn bg-warning-700 btn-icon addition-leaf-controls__delete">
